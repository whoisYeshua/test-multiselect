...
import { CurrencyType } from '../../accessories/CurrencyFormatter/CurrencyFormatter';
export interface AllowedSymbolsRegexParams {
    pattern: string | RegExp;
    flag: string;
}
export declare enum AutoCorrection {
    NOUN = "properNoun",
    UPPER_CASE = "properUpperCase",
    RUS_TO_ENG = "properRusToEng",
    ENG_TO_RUS = "properEngToRus",
    EQUALS_RUS_TO_ENG = "properEqualsRusToEng"
}
export declare type AutoCorrectionType = `${AutoCorrection}`;
export interface InputProps extends ControlWithCustomButtonProps, Omit<RangeInputExternalProps, 'caption'> {
     ...
     * `properEqualsRusToEng` - заменяет буквы, введённые кириллицей на схожие по написанию латинские,
     * независимо от регистра (хотя в нижнем регистре некоторые и не схожи вовсе).
     * При этом удаляет те, для которых нет аналогов.
     */
    ...
    /**
     * Если указан и не равен `0`, добавляет задержку вызова обработчика `onChange` в миллисекундах,
     ...
     * Только для режима currencyView.
     */
...
declare const Input: {
    ...
    defaultProps: {
        ...
        scrollTopBreakpoint?: import("@ui/screen-size-provider").ScreenWidthShortcut;
        scrollTopOffset?: number;
        screenWidthShortcut?: import("@ui/screen-size-provider").ScreenWidthShortcutWithUnknown;
        className?: string;
        hidden?: boolean;
        id?: string;
        disabled?: boolean;
        tooltipIconVariant?: "default" | "filled";
        locked?: boolean;
        name?: string;
